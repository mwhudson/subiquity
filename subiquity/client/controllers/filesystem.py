...
class FilesystemController(SubiquityTuiController, FilesystemManipulator):
    ...
    async def start_ui_real(self, status):
        self.model = FilesystemModel(getattr(Bootloader, status['bootloader']))
    ...
    async def _reset(self):
        status = await self.app.post('/storage/reset', {})
    ...
    def finish(self):
        ...
        self.app.next_screen(self.post(self.model._render_actions()))
