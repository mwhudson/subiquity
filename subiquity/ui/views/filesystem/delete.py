...
def can_delete(obj, obj_desc=_("it")):
    if isinstance(obj, _Device):
        for p in obj.partitions():
            ok, reason = can_delete(p, obj_desc=_("partition {}").format(p._number))
            if not ok:
                return False, reason
    ...
    if isinstance(cd, Raid):
        ...
        if len(cd.devices) > l.min_devices:
            ...
            reason = _("deleting {obj} would leave the {desc} {label} with less than {min_devices} devices.").format(
                obj=_(obj_desc), desc=cd.desc(), label=cd.label, min_devices=l.min_devices)
            return False, reason
    else:
...
def delete_consequences(controller, obj, obj_desc=_("It")):
    ...
    deleter = ("delete {} {}".format(obj.type, obj.label), make_device_deleter(controller, obj))
    if isinstance(obj, _Device):
        if len(obj.partitions()) > 0:
            ...
            for p in obj.partitions():
                ...
                new_lines, new_delete_funcs = delete_consequences(controller, p, desc)
                lines.extend(new_lines)
                lines.append("")
                delete_funcs.extend(new_delete_funcs)
            return lines[:-1], delete_funcs + [deleter]
        unused_desc =  _("{} is not formatted, partitioned, or part of any constructed device.").format(obj_desc)
    else:
        unused_desc =  _("{} is not formatted or part of any constructed device.").format(obj_desc)
    fs = obj.fs()
    cd = obj.constructed_device()
    if fs is not None:
        ...
        if isinstance(cd, Raid):
            delete_funcs = [("remove {} from {}".format(obj.label, cd.name), make_device_remover(cd, obj)), deleter]
            return [_("{} is part of the {} {}. {} will be left with {} devices.").format(
                obj_desc, cd.desc(), cd.label, cd.label, len(cd.devices) - 1)], delete_funcs
        else:
            raise Exception("unexpected constructed device {}".format(cd.label))
    else:
...
class ConfirmDeleteStretchy(Stretchy):
    ...
    def __init__(self, parent, obj):
        ...
        if delete_ok:
            ...
            new_lines, delete_funcs = delete_consequences(self.parent.controller, obj)
            log.debug("consequences %s \n + %s", new_lines, "\n + ".join(map(repr,delete_funcs)))
            lines.extend(new_lines)
            self.delete_funcs = delete_funcs
        else:
            title = "Cannot delete {}".format(obj.desc())
            lines = [_("Cannot delete {} because {}").format(obj.label, reason)]
        delete_btn = danger_btn(label=_("Delete"), on_press=self.confirm)
        if not delete_ok:
            delete_btn = WidgetDisable(Color.info_minor(delete_btn.original_widget))
        widgets = [
            Text("\n".join(lines)),
            Text(""),
            button_pile([
